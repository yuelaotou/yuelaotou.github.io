...
If Trim(Request.QueryString("Action"))="Lock" Then
	...
	Call Lock_OK("Link",ID,ReCord,Locks,"?SS_ID="&SS_ID&"")
End if 
...
Sql="select * from Link Where SS_ID="&SS_ID&" order by id asc"
...
%>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#D7DEF8">
        <tr bgcolor="#FFFFFF">
          ...
          <td width="30" align="center" bgcolor="#FFFFFF">链接</td>
 ...
 %>
        <tr bgcolor="#FFFFFF" style="cursor:hand" OnMouseOver="this.bgColor='#F0F0F0'" OnMouseOut="this.bgColor='#FFFFFF'">
          ...
          <td width="88" align="center"><% If Rs("Img")<>Empty Then %>
              <img src="../UploadFile/<%= Rs("Img") %>" alt="" width="88" height="31">
            <% End If %></td>
          <td>&nbsp;<%= rs("Title") %></td>
          <td align="center"><%= rs("DateTime") %></td>
          <td align="center"><%= rs("Content") %></td>
          <td align="center"><a href="<%= rs("URL") %>" target="_blank"><img src="../sysImages/IE.gif" width="19" height="18" border="0" align="absmiddle"></a></td>
          <td align="center"><% If Rs("Img")<>Empty Then %>
              <img src="../sysImages/img.gif" width="20" height="20" border="0" align="absmiddle" onClick="javascript:newimg('<% If Left(rs("Img"),7)="http://" Then %><%= rs("Img") %><% Else %>
../UploadFile/<%= rs("Img") %><% End If %>');">
              <% End If %>
          </td>
          <td align="center"><a href="Link_Edit.asp?SS_ID=<%= SS_ID %>&ID=<%= rs("ID") %>"><img src="../sysImages/Edit.gif" width="12" height="12" border="0" alt="修改"></a> </td>
...
%>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td align="center"><input type="button" name="Submit_OK" value="添加" onClick="location='Link_Insert.asp?SS_ID=<%= SS_ID %>'">
            &nbsp;
