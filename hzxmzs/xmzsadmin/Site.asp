...
If Trim(Request.QueryString("Action"))="Lock" Then
	...
	Call Lock_OK("Site",ID,ReCord,Locks,"?SS_ID="&SS_ID&"")
End if 
 ...
 %>
        <tr bgcolor="#FFFFFF" style="cursor:hand" OnMouseOver="this.bgColor='#F0F0F0'" OnMouseOut="this.bgColor='#FFFFFF'">
          ...
          <td width="88" align="center"><% If Rs("Img")<>Empty Then %>
              <img src="../UploadFile/<%= replace(Rs("Img"),".","a.") %>" alt="" width="120" height="100">
            <% End If %></td>
          <td align="center"><%= rs("name") %></td>
          <td align="center"><%= rs("area") %></td>
          <td align="center"><%= rs("price") %></td>
          <td align="center"><%= rs("designer") %></td>
          <td align="center">
			<%If rs("status") = 1 Then  response.write "施工前期"%>
			<%If rs("status") = 2 Then  response.write "施工中期"%>
			<%If rs("status") = 3 Then  response.write "施工后期"%>
			<%If rs("status") = 4 Then  response.write "工程结束"%>
		  </td>
          <td align="center"><a href="Site_Edit.asp?SS_ID=<%= SS_ID %>&ID=<%= rs("ID") %>"><img src="../sysImages/Edit.gif" width="12" height="12" border="0" alt="修改"></a> </td>
...
%>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td align="center"><input type="button" name="Submit_OK" value="添加" onClick="location='Site_Insert.asp?SS_ID=<%= SS_ID %>'">
            &nbsp;
